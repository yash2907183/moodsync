...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Check if we should use GPU (set to False by default for Mac/CPUs)
        use_gpu = os.getenv("USE_GPU", "False").lower() == "true"
        # Pre-load the models so the first request isn't slow
        get_sentiment_analyzer(use_gpu=use_gpu)
        logger.info("Sentiment analyzer initialized successfully")
    except Exception as e:
        logger.warning(f"Sentiment analyzer could not start (ImportError?): {e}")
        logger.warning("Continuing without pre-loaded models...")
...
from app.api import auth, tracks, insights, analysis, mood, summary, playlists, cron, research
